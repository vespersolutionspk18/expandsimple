    ...
    <>
      ...
      {isOpen && (
        <div className="fixed bottom-24 right-6 w-96 h-[600px] bg-white rounded-2xl shadow-2xl flex flex-col z-50 border border-gray-200">
          {/* Header */}
          <div className="bg-[#1d4ed8] text-white p-4 rounded-t-2xl flex justify-between items-center">
            <h3 className="font-semibold text-lg">Chat with Cindy</h3>
            <button
              onClick={toggleChat}
              className="text-white hover:bg-[#1e40af] rounded-full p-1 transition-colors"
            >
              ...
              >
                ...
                {message.role === 'assistant' && shouldShowBookingButton(message.content, message.role) && (
                  <button
                    ...
                    className="bg-[#1d4ed8] text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-[#1e40af] transition-colors flex items-center gap-2"
                  >
          ...
          <div className="p-4 border-t border-gray-200">
            <div className="flex space-x-2">
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-[#1d4ed8] focus:border-transparent disabled:bg-gray-100 text-gray-800"
              />
              <button
                onClick={handleSend}
                disabled={isLoading || !input.trim()}
                className="bg-[#1d4ed8] text-white p-2 rounded-full hover:bg-[#1e40af] transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <button
        onClick={toggleChat}
        className="fixed bottom-6 right-6 bg-[#1d4ed8] text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all hover:scale-110 z-50"
